...
metadata:
  hermes:
    ...
    related_skills: [deep-search-cli, duckduckgo-search, mcporter]
---
...
### Backend selection for Deep Search-style local routers
When comparing Parallel with Tavily or similar providers for a single `DEEP_SEARCH_WEB_SEARCH_ENDPOINT`-style integration, treat Parallel Search and Parallel Task as different primitives. For operating the local `deep-search-mcp` CLI itself, load `deep-search-cli`; this section is only the Parallel vendor/API boundary reference.
- Parallel Search API is a strong default backend for cheap production search, but needs a local adapter because it uses POST JSON plus `x-api-key` auth rather than plain `GET ?q=&limit=`.
- Parallel Task API is not a web-search endpoint; use it for async structured research, enrichment, cited reports, confidence/reasoning, and P2 orchestration.
- Keep Tavily available for search+extract/content-depth workflows, and keep SearXNG as a no-key local fallback.
- Normalize all search backends to `{results:[{title,url,snippet}]}` before exposing them to simple endpoint consumers.

See `references/parallel-vs-tavily-deep-search-endpoint.md` for cost comparison and router-fit notes.
